...
const CATEGORIES = [
  {
    ...
    icon: "coffee",
    keywords: ["chai", "food", "milk", "khana", "nasta", "hotel", "dinner", "tea", "sabji"],
  },
  {
    id: "transport",
    name: "TRANSPORT",
  ...
  },
  {
    id: "health",
    name: "HEALTH",
    detail: "Doctor, Dawali",
    icon: "heart",
    keywords: ["doctor", "medicine", "med", "hospital", "dawali"],
  },
  {
    id: "other",
    name: "OTHER",
...
export default function DashboardScreen({ navigation }: any) {
  ...
  // --- LOGIC: HIGH-PERFORMANCE AUTO-PARSING ---
  ...
  // --- LOGIC: SOVEREIGN SHARE PROTOCOLS ---
  const handleShareCode = async () => {
    try {
      await Share.share({
        message: `🏆 Join GLOBAL WALLET! Track wealth and earn ₹ passive income. Use my code: ${user?.mobile_number}`,
      });
  ...
  const handleShareReport = async () => {
    if (user?.subscription_status !== "premium")
      return Alert.alert("⭐ Premium Only", "Upgrade to share executive reports.");
    const report = expenses
      .map((e, i) => `#${expenses.length - i} | ${e.category.toUpperCase()}\nNote: ${e.description || "Cash"}\n₹${e.amount} | ${new Date(e.created_at).toLocaleTimeString()}`)
      .join("\n\n");
    await Share.share({
      message: `💎 GLOBAL WALLET REPORT\nUser: ${user.full_name}\n\n${report}\n\nTOTAL: ₹${filteredLedgerSum.toLocaleString("en-IN")}`,
    });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={styles.brandContainer}>
          ...
          <Text style={styles.statusText}>{user?.subscription_status === "premium" ? "⭐ PREMIUM MEMBER" : "FREE ACCOUNT"}</Text>
        </View>
      ...
      <View style={styles.actionRow}>
        <Pressable style={styles.actionBtn} onPress={handleShareCode}>
          <Feather name="user-plus" size={14} color="#FFD700" />
          <Text style={styles.btnText}>SHARE CODE</Text>
        </Pressable>
        <Pressable style={styles.actionBtn} onPress={handleShareReport}>
              ...
              <View style={styles.col1}>
                ...
                <View>
                  ...
                  <Text style={styles.itemNote} numberOfLines={1}>
                    {item.description || "Cash Entry"} • {new Date(item.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                  </Text>
                </View>
              </View>
          ...
          <View style={[styles.modalContent, { paddingBottom: insets.bottom + 20 }]}>
            {modalMode === "upgrade" ? (
              <View style={styles.upgradeContent}>
                ...
                <Pressable onPress={() => setShowModal(false)} style={{ marginTop: 20 }}><Text style={{ color: "#FF4B2B" }}>Maybe Later</Text></Pressable>
              </View>
            ) : modalMode === "withdraw" ? (
                <View style={styles.upgradeContent}>
                  <Text style={styles.modalTitle}>Request Payout</Text>
                  <Text style={styles.upDesc}>Available Balance: ₹{user?.wallet_balance}</Text>
                  <TextInput style={styles.detailInput} placeholder="Enter UPI ID for Payout" placeholderTextColor="#666" value={upiId} onChangeText={setUpiId} />
              ...
              <>
                <View style={styles.modalHeader}><Text style={styles.modalTitle}>{editingId ? "Update Record" : "New Entry"}</Text><Pressable onPress={() => setShowModal(false)}><Feather name="x" size={28} color="white" /></Pressable></View>
                <TextInput style={styles.amountInput} placeholder="₹ 0.00" placeholderTextColor="#444" keyboardType="numeric" autoFocus value={amount} onChangeText={setAmount} />
...
const styles = StyleSheet.create({
  ...
  col1: { flex: 2.2, flexDirection: "row", alignItems: "center" },
  ...
  feeAmount: { color: "white", fontSize: 48, fontWeight: "900", marginBottom: 35 },
});
